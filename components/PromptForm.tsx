...
const modeIcons: Record<GenerationMode, React.ReactNode> = {
  ...
  [GenerationMode.STORY_TO_VIDEO]: <StoryModeIcon className="w-5 h-5" />,
  [GenerationMode.EXTEND_VIDEO]: <FilmIcon className="w-5 h-5" />,
};

const DEFAULT_STORY_SCRIPT = JSON.stringify(
  {
    scene: 1,
    time: '0–5 sec',
    dialogue: 'अरे! ये क्या? हल्क जंगल में आ गया! 😲',
    voice_tone: 'मस्त बच्चा (चकित और cute)',
    image_prompt:
      '3D realistic Hulk walking in a colorful jungle, surprised look, monkeys on trees watching curiously, sunlight through leaves, bright colors, 9:16 vertical frame',
  },
  null,
  2,
);
...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  const handleSelectMode = (mode: GenerationMode) => {
    ...
    if (mode === GenerationMode.STORY_TO_VIDEO) {
      setPrompt(DEFAULT_STORY_SCRIPT);
    } else if (generationMode === GenerationMode.STORY_TO_VIDEO) {
      setPrompt('');
    }
  };
  ...
  const promptPlaceholder = {
    ...
    [GenerationMode.STORY_TO_VIDEO]:
      'Enter your story script in JSON format...',
  ...
  const renderMediaUploads = () => {
    ...
    if (generationMode === GenerationMode.EXTEND_VIDEO) {
      // If a video is selected for extension, show a larger preview.
      if (inputVideo) {
        return (
          <div className="mb-3 p-4 bg-[#2c2c2e] rounded-xl border border-gray-700 flex flex-col items-center justify-center gap-4">
            <p className="text-sm text-gray-400 font-medium self-start px-2">
              Video to be extended:
            </p>
            <div className="relative w-full max-w-lg aspect-video rounded-lg overflow-hidden bg-black shadow-inner">
              <video
            ...
            <button
              ...
              className="px-4 py-2 bg-gray-600 hover:bg-gray-500 text-white text-sm font-semibold rounded-lg transition-colors flex items-center gap-2">
              <XMarkIcon className="w-4 h-4" />
              Change Video
            </button>
          </div>
  ...
  const isStoryMode = generationMode === GenerationMode.STORY_TO_VIDEO;
  ...
  switch (generationMode) {
    ...
    case GenerationMode.REFERENCES_TO_VIDEO:
      ...
      break;
    case GenerationMode.STORY_TO_VIDEO:
      if (!prompt.trim()) {
        isSubmitDisabled = true;
        tooltipText = 'Please enter a script.';
      } else {
        try {
          JSON.parse(prompt);
        } catch (e) {
          isSubmitDisabled = true;
          tooltipText = 'Script must be valid JSON.';
        }
      }
      break;
      ...
      <form onSubmit={handleSubmit} className="w-full">
        ...
        <div className="flex items-end gap-2 bg-[#1f1f1f] border border-gray-600 rounded-2xl p-2 shadow-lg focus-within:ring-2 focus-within:ring-indigo-500">
          ...
          <textarea
            ...
            className={`flex-grow bg-transparent focus:outline-none resize-none text-base text-gray-200 placeholder-gray-500 py-2 ${
              isStoryMode ? 'min-h-48 font-mono' : 'max-h-48'
            }`}
            rows={isStoryMode ? 8 : 1}
          />
          <button
            type="button"
